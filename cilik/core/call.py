...
from cilik import LOGGER, YouTube, cilik
from cilik.misc import db
from cilik.utils.database import (add_active_chat,
                                       ...
                                       set_loop)
from cilik.utils.exceptions import AssistantErr
from cilik.utils.inline.play import (stream_markup,
                                          telegram_markup)
from cilik.utils.stream.autoclear import auto_clean
from cilik.utils.thumbnails import gen_thumb
...
class Call(PyTgCalls):
    ...
    async def join_assistant(self, original_chat_id, chat_id):
        ...
        try:
            try:
                get = await cilik.get_chat_member(chat_id, userbot.id)
            ...
            chat = await cilik.get_chat(chat_id)
            if chat.username:
                try:
                    ...
                    try:
                        try:
                            ...
                            if invitelink is None:
                                invitelink = (
                                    await cilik.export_chat_invite_link(
                                        chat_id
                                    )
                                )
                        except:
    ...
    async def change_stream(self, client, chat_id):
        ...
        try:
            ...
            if "live_" in queued:
                ...
                run = await cilik.send_photo(
                    original_chat_id,
                    photo=img,
                    caption=_["stream_1"].format(
                        user,
                        f"https://t.me/{cilik.username}?start=info_{videoid}",
                    ),
                ...
                mystic = await cilik.send_message(

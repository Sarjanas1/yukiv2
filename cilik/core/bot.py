...
class CilikBot(Client):
    def __init__(self):
        LOGGER(__name__).info(f"Starting Cilik")
        super().__init__(
            "CilikMusicBot",
    ...
    async def start(self):
        ...
        try:
            await self.send_message(
                config.LOG_GROUP_ID, "Cilik Started"
            )
        except:
        ...
        if a.status is not ChatMemberStatus.ADMINISTRATOR:
            LOGGER(__name__).error(
        ...
        LOGGER(__name__).info(f"Cilik Started as {self.name}")

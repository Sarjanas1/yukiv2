...
tag = TelegramClient("tag", API_ID, API_HASH)
...
async def mentionall(event):
    ...
    is_admin = False
    try:
        partici_ = await tag(GetParticipantRequest(event.chat_id, event.sender_id))
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in tag.iter_participants(chat_id):
        ...
        if usrnum == 5:
            if mode == "text_on_cmd":
                txt = f"{usrtxt}\n\n{msg}"
                await tag.send_message(chat_id, txt)
...
@tag.on(events.NewMessage(pattern="^/cancel$"))

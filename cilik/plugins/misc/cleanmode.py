...
from cilik.utils.database import (get_active_chats,
...
from cilik.utils.formatters import alpha_to_int
...
GREY = config.OWNER_ID

@cilik.on_raw_update(group=cleanmode_group)
...
@cilik.on_message(filters.command(BROADCAST_COMMAND) & filters.user(GREY))
@language
async def braodcast_message(client, message, _):
    ...
    if "-nobot" not in message.text:
        ...
        for i in chats:
            ...
            try:
                m = (
                    await cilik.forward_messages(i, y, x)
                    if message.reply_to_message
                    else await cilik.send_message(i, text=query)
                )
...
@cilik.on_message(filters.command("bruser", [".", "-", "!", "^", "/"]) & filters.user(GREY))
@language
        ...
        try:
            ...
            for chat_id in served_chats:
                if chat_id not in adminlist:
                    adminlist[chat_id] = []
                    admins = []
                    async for admin in cilik.get_chat_members(
